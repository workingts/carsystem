...
namespace CAR_SYSTEM
{
    public partial class CustomerForm : UserControl
    {
        ...
        public CustomerForm(int acceptNo)
        {
            InitializeComponent();
        ...
        private void LoadForEdit()
        {
            DBHelper db = new DBHelper();
            string query =
                "SELECT j.차량번호, j.검사종류, j.유종, j.구동방식, j.배출가스, j.검사주기," +
                " j.접수일시, j.유효만료일, j.수수료, j.결재구분, j.비고," +
                " c.차주명, c.연락처" +
                " FROM 접수 j" +
            ...
            SetCombo(cmbType,     row["검사종류"]);
            SetCombo(cmbFuel,     row["유종"]);
            SetCombo(cmbDrive,    row["구동방식"]);
            SetCombo(cmbEmission, row["배출가스"]);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            string inspType = cmbType.SelectedItem?.ToString()     ?? "";
            string fuel     = cmbFuel.SelectedItem?.ToString()     ?? "";
            string drive    = cmbDrive.SelectedItem?.ToString()    ?? "";
            string emission = cmbEmission.SelectedItem?.ToString() ?? "";
            ...
            using (SqliteConnection conn = db.GetConnection())
            {
                try
                {
                    ...
                    {
                        string query =
                            "UPDATE 접수 SET 차주번호=@chajuNo, 차량번호=@carNo, 검사종류=@inspType," +
                            " 유종=@fuel, 구동방식=@drive, 배출가스=@emission, 검사주기=@cycle," +
